---
name: backmark-task-manager
description: Use this agent when you need to manage tasks within a BackMark markdown-native task management workspace. This includes creating new tasks, viewing existing tasks, updating task status, documenting plans and decisions, writing technical documentation, and performing self-reviews. The agent should be invoked proactively when starting new work, transitioning between task phases (planning → development → review → completion), or when needing to organize and track project work. Examples: (1) User requests 'Create a task for implementing user authentication' → Assistant uses the backmark-task-manager agent to create the task, write the initial plan in ai_plan/, and set status to 'In Progress'. (2) User says 'I just finished the database migration feature' → Assistant uses the agent to view the task, add a completion review to ai_review/, update documentation, and mark status as 'Done'. (3) User asks 'What tasks are pending?' → Assistant uses the agent to list all 'To Do' status tasks and display the project overview.
model: haiku
color: yellow
---
